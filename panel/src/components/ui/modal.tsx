...
const ResponsiveModalVariants = cva(
    cn(
        'fixed z-50 gap-4 border border-zinc-200 bg-white sm:rounded-lg dark:border-zinc-800 dark:bg-zinc-950 p-6 shadow-lg transition ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-300 data-[state=open]:duration-500 overflow-y-auto',
...
>(({ side = 'bottom', className, children, ...props }, ref) => (
    <ResponsiveModalPortal>
        ...
        >
            {children}
            <ResponsiveModalClose className="absolute right-4 top-4 rounded-sm opacity-70 ring-offset-zinc-950 transition-opacity hover:opacity-100 focus:outline-none focus:ring-0 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none data-[state=open]:bg-secondary">
                <X className="h-4 w-4" />
...
>(({ className, ...props }, ref) => (
    <DialogPrimitive.Title
        ref={ref}
        className={cn("dark:text-zinc-50 text-lg font-semibold leading-none tracking-tight", className)}
        {...props}
    />
));
